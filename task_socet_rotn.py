...
class Server(object):
    ...
    def make_response(self, data, rotn):
        ...
        for i in __data:
            chars = string.ascii_uppercase if i.isupper() else string.ascii_lowercase

            if i.ischars():
                __result += chars[(chars.index(i) + __rotn) % len(chars)]
            else:
                __result += i
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Кодировщик')
