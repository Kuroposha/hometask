...
def action_edit_t():# УРА! это дерьмо наконец то заработало!
    ...
    print('\nЗадача отредактированна!')


def action_replay_t():# УРА! это дерьмо наконец то заработало!
    """Начать задачу заново
    Идея в том, что нельзя начать заново то. что еще не закончено.
    И пока я писала предыдущие тексты
...
def action_close_t():# УРА! это дерьмо наконец то заработало!
    """Завершить задачу
    Идея в том, что завершить можно только открытую задачу. Поэтому нужна
    выборка по открытым задачам и одновременно с этим по имени или айди задачи.
    НУжно предложить пользователю Вывести айди, имя или список открытых задач."""
